...
1. Add the environment variables to your shell configuration files inside [_home directory_](./home-directory.md), The file name depends on which shell you are using.
   - For `bash`, use `~/.bashrc`
   - For `zsh`, use `~/.zshrc`
...
nano ~/.bashrc # Using Nano
nvim ~/.bashrc # Using Neovim
```

**Write the [export](./export-command.md) command at the very bottom of the file:**

```.bashrc
# Syntax: export <variable-name>=<value>
export EDITOR=nvim

# Syntax: export PATH="$PATH:/path/to/new/directory"
export PATH="$PATH:~/workspace/script/bin"
```

2. To apply the changes immediately without re-opening your terminal, use the [`source` command](./source-command.md) to reload the configuration file:
